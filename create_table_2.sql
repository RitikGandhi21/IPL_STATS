...
COPY player FROM 'D:\\academia\\col362\\project final\\ipl\\ipl-data-till-2017\\Player.csv' delimiter ',' csv header;
...
COPY match FROM 'D:\\academia\\col362\\project final\\ipl\\ipl-data-till-2017\\Match.csv' delimiter ',' csv header;
...
COPY ball FROM 'D:\\academia\\col362\\project final\\ipl\\ipl-data-till-2017\\Balls.csv' delimiter ',' csv header;

CREATE TABLE teams(Team_SK int, Team_Id int, Team_Name text);
COPY teams FROM 'D:\\academia\\col362\\project final\\ipl\\ipl-data-till-2017\\Team.csv' delimiter ',' csv header;
...
COPY batting_stats FROM 'D:\\academia\\col362\\project final\\ipl\\ipl-data-till-2017\\batting_stats.csv' delimiter ',' csv header;
...
COPY bowling_stats FROM 'D:\\academia\\col362\\project final\\ipl\\ipl-data-till-2017\\bowling_stats.csv' delimiter ',' csv header;
...
create view per_striker as select striker, count(match_id) filter(where runs>=100) as Hundreds, count(match_id) filter(where runs = 0) as ducks,
count(match_id) filter(where runs>=50 and runs<100) as Fiftys, count(match_id) as matches, max(runs) as top_score,
AVG(runs) as Average from match_stats group by striker order by striker;



select player_name , hundreds, ducks, fiftys, top_score from per_striker,player where striker = player_id and hundreds>4 order by ducks; 


create view bowler_out as select match_id, bowler, count(player_out) as out 
 from ball group by (match_id,bowler) order by bowler;

create view bowler_max2 as select bar.match_id, bowler, m from
(select match_id , max(out) as m from bowler_out group by(match_id)) as bar, bowler_out where bowler_out.out = bar.m and 
bowler_out.match_id = bar.match_id order by m desc; 



create view striker_run as select match_id, striker, count(runs_scored) as runs 
 from ball group by (match_id,striker) order by striker;

create view striker_max2 as select bar.match_id, striker, m from
(select match_id , max(runs) as m from striker_run group by(match_id)) as bar, striker_run where striker_run.runs = bar.m 
and striker_run.match_id = bar.match_id order by m desc;


select striker_max2.match_id, striker, player_name, striker_max2.m as runs, bowler_max2.m as wickets from striker_max2, bowler_max2, Player where striker = bowler 
  and striker_max2.match_id = bowler_max2.match_id and striker = Player.Player_Id ;  
...
select match_id,bowler,player_name , m  from bowler_max, player where player_id = bowler;

  select match_id , striker , max(runs) from 
  (select match_id, striker, sum(runs_scored) as runs 
   from ball group by (match_id,striker) order by striker) as foo group by(match_id, striker); 
